...
function SectionTitle({ eyebrow, title, italic, children }: { eyebrow: string; title: string; italic?: string; children?: React.ReactNode }) { return <div className="section-title reveal-on-view"><Kicker left={eyebrow} /><h2>{title}</h2>{italic && <p className="deck-italic">{italic}</p>}{children && <p className="lede">{children}</p>}</div>; }
function DetailDrawer({ open, onClose, title, children }: { open: boolean; onClose: () => void; title: string; children: React.ReactNode }) { if (!open) return null; return <div className="drawer-backdrop" onClick={onClose}><aside className="detail-drawer" onClick={(e) => e.stopPropagation()}><button className="drawer-close" onClick={onClose} aria-label="Close"><X size={18}/></button><Kicker left="Investor detail" right="click outside to close" /><h3>{title}</h3>{children}</aside></div>; }

function Hero() { return <section id="top" className="split-hero"><div className="hero-copy reveal-on-view"><Kicker left="CIARRA PARDO CO · 2026" right="CONFIDENTIAL" /><p className="deck-italic hero-italic">Heritage is medicine.</p><h1>CIARRA<br />PARDO</h1><p className="source-code"><TypeLine text="the ecosystem’s source code" /></p><p className="hero-sub">MI OJO · PEPPERS & BELI · BEAUTYDAYS · THE RESIDENCES</p><div className="hero-actions"><a href="#portfolio" className="gold-btn">Explore the ecosystem <ArrowRight size={16} /></a><a href="#financials" className="line-btn">View investor model</a></div></div><div className="hero-image"><Image src="/assets/canonical/page01_img02.png" alt="Ciarra Pardo black and white founder portrait" fill priority sizes="(max-width: 900px) 100vw, 44vw" className="object-cover grayscale" /></div></section>; }
